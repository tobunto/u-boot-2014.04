...
libs-y += drivers/spi/
libs-y += drivers/regulator/
libs-$(CONFIG_JZ_SFC) += drivers/mtd/devices/
...
ALL-y += u-boot-with-spl-mbr.bin
else
ALL-y += u-boot-with-spl.bin
endif

ifeq ($(CONFIG_GPT_CREATOR),y)
ALL-y += u-boot-with-spl-mbr-gpt.bin
else
ALL-y += u-boot-with-spl.bin
endif
...
u-boot-with-spl-mbr.bin: u-boot-with-spl.bin
	tools/mbr_creator	\
		...
		-o mbr.bin > /dev/zero
	cat mbr.bin u-boot-with-spl.bin > $@
	rm mbr* -rf

u-boot-with-spl-mbr-gpt.bin: u-boot-with-spl.bin
	tools/gpt_creator $(CONFIG_GPT_TABLE_PATH)/partitions.tab mbr-of-gpt.bin gpt.bin
	cat mbr-of-gpt.bin gpt.bin > mbr-gpt.bin
	dd if=/dev/zero of=tmp.bin bs=512 count=33
	cat mbr-gpt.bin tmp.bin > tmp2.bin
	dd if=tmp2.bin of=mbr-gpt.bin bs=512 count=34
ifneq ($(CONFIG_GPT_AT_TAIL),y)
	cat mbr-gpt.bin u-boot-with-spl.bin > $@
else
	chmod +x tools/ingenic-tools/mk-gpt-xboot.sh
	tools/ingenic-tools/mk-gpt-xboot.sh mbr-of-gpt.bin \
	u-boot-with-spl.bin gpt.bin $(CONFIG_GPT_TABLE_PATH)/partitions.tab $@
endif
	rm tmp* mbr* gpt* -rf
...
CLEAN_FILES += u-boot.lds include/bmp_logo.h include/bmp_logo_data.h \
	       ...
	       include/tpl-autoconf.mk tools/*creator tools/spi_checksum
