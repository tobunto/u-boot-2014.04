...
void sdram_init(void)
{
	...
	/* setup DDR clock */
	ddr_cdr = CONFIG_SYS_APLL_FREQ / CONFIG_SYS_MEM_FREQ - 1;
	writel(ddr_cdr | CPM_DDRCDR_DCS_SCLKA | CPM_DDRCDR_CE, CPM_BASE + CPM_DDRCDR);
	...
	if(bypass)
		cpm_outl(cpm_inl(CPM_DDRCDR) | 1 << 26, CPM_DDRCDR);
}
