...
#include <asm/arch/sfc_timing_val.h>
...
//	clk_set_rate(SFC, CONFIG_SFC_NOR_RATE);
	...
	/* setup SFC clock */
	ssi_cdr = CONFIG_SYS_MPLL_FREQ / CONFIG_SFC_NOR_RATE - 1;
	writel(ssi_cdr | CPM_SSICDR_SSIPCS_MPLL | CPM_SSICDR_CE, CPM_BASE + CPM_SSICDR);
	while (readl(CPM_BASE + CPM_SSICDR) & CPM_SSICDR_SSI_BUSY);
...
int sfc_read_data(unsigned int from, unsigned int len, unsigned int *buf)
{
	...
	while(len) {
		tmp_len = sfc_do_read(from + current_len, (unsigned char *)&buf[current_len], len);
		current_len += tmp_len;
		len -= tmp_len;
	}
...
#endif
#endif
	spi_nor_info = (struct mini_spi_nor_info *)(nor_info + sizeof(struct spinor_flashinfo));
